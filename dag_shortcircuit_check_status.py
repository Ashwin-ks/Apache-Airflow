...
dag = DAG(
    'dag_shortcircuit_check_status', default_args=default_args, schedule_interval=schedule_interval,
    ...
    try:
        ...
        if all(state=="success" for state in dag_state_all):
            ...
            while any(state=="running" for state in dag_state_all):
                logging.info('Current status of dependent dags',dag_state_all)
                logging.info('Checking running dags status after 120 seconds')
                time.sleep(120)
                dag_state_all=[check_dag_state(dag) for dag in ('dag_schedule_test1','dag_schedule_test2','dag_test')]
                logging.info('Dependent dags status after fixed interval',dag_state_all)
...
check_prev_dag_state.set_downstream(task_1_session2_py)
