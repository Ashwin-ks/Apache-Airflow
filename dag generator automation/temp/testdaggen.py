...
def get_trig_params(file):
    ...
    for each_section in config.sections():
        ...
        section_params={}
        for (each_key, each_val) in config.items(each_section):
	    print(each_key,'IIIIIIIIIIIIIII',each_val)
            if each_key=='trigger':
                print ("each key,each value = " + each_key.upper() , each_val)
                section_params.update({each_key.upper(): each_val})
    ...
    for dir in os.walk(workflows_dir).next()[1]:
        ...
        if dir.startswith(workflow_dir_prefix):
            ...
            for profile, params in task_params_dict.items():
                ...
                if file_suffix == "sql":
                     generate_dag(dag_name=dag_name, script_files=script_files,dag_template="SessionMainDagTemplate.py.jinja", task_params=params,trig_parameter=trig_params_dict[profile],workflow_name=workflow_name,profile_param=profile)
...
def generate_trigoperator_snippet(trig_params,task,profile_param):
    snippet=j2_env.get_template('TestSessionDagRunOperator.py.jinja').render(
                TRIGGER_DAG_ID=trig_params['TRIGGER'],
...
def generate_dag(dag_name, script_files, dag_template, task_params, trig_parameter,profile_param, workflow_name):
    ...
    for i in range(1, len(task_order_details.keys()) + 1):
        ...
        if i == len(task_order_details.keys()) and "TRIGGER" in trig_parameter:
            print '??????????????????????????????????????????????'
