	...
	trig_params_dict = {"": None}
		...
		########################################################################
            ...
            for profile, params in task_params_dict.items():
                ...
                if file_suffix == "sql":
		     generate_dag(dag_name=dag_name, script_files=script_files,dag_template="SessionMainDagTemplate.py.jinja", task_params=params,trig_parameter=trig_params_dict,workflow_name=workflow_name)
...
def generate_trigoperator_snippet(trig_params,task):
...
def generate_dag(dag_name, script_files, dag_template, task_params, trig_parameter, workflow_name):
    print '*******************          generte dag     **************************'
    ...
    for i in range(1, len(task_order_details.keys()) + 1):
        ...
        if i == len(task_order_details.keys()) and "TRIGGER_DAG_ID" in trig_parameter:
            print '??????????????????????????????????????????????'
            dag+=generate_trigoperator_snippet(trig_parameter,all_tasks)  
	############################################
