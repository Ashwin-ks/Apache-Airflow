...
RqstId = ''
try:
    with open(config.INPUT_DIR+'/'+config.TABLE_NAME+'_'+'reqlog_all.txt', 'r') as requestid:
...
rstatusRqstId=''
try:		
    with open(config.INPUT_DIR+'/'+config.TABLE_NAME+'_'+'reqlog_r.txt', 'r') as rstatuslog:
...
oracleoutput = open(config.ENVIRONMENT_PATH+config.ORACLE_SQL_OUTPUTFILE,'w')
os.chmod(config.ENVIRONMENT_PATH+config.ORACLE_SQL_OUTPUTFILE,0o777)												
### Generate Sql
try:
    with open (config.ENVIRONMENT_PATH+config.ORACLE_SQL_INPUTFILE, 'r') as oracleinput:
...
try:
    with open (config.ENVIRONMENT_PATH+config.TABLE_NAME+'_'+'param_in.txt') as paramin:
        for line in paramin:
