...
import airflow
start_date = datetime(year=2017, month=6, day=21, hour=20, minute=55)
task_concurrency=15
...
dag = DAG(
    'EIS_T_OutbndShpmtHdr_P', default_args=default_args, schedule_interval=schedule_interval,
...
set NBR_OF_SOURCE_ROWS=(Select count(*) from EIS_STG_T.OutbndShpmtHdr_P where ROWLASTUPDTFK = (select ETL_TABLE_AUDIT_PK from EIS_T.A_ETL_TABLE_AUDIT
where table_name=upper('OutbndShpmtHdr[ASIA]') and TBL_ETL_STATUS='In Progress'))
where table_name=upper('OutbndShpmtHdr[ASIA]') and TBL_ETL_STATUS='In Progress';
...
task_3_c_sql = SnowFlakeOperator(
    ...
    parameters={'SCHEDULE_INTERVAL': "'0 24 * * *'", 'START_DATE': 'datetime(year=2017, month=6, day=21, hour=20, minute=55)', 'TGTDB': 'EIS_T', 'SOURCE_SYSTEM': '3', 'STGDB': 'EIS_STG_T', 'TABLE_NAME_IN_AUDIT': 'OutbndShpmtHdr[ASIA]', 'VIEW_NAME': 'VTTK_A_xi_V_PRA', 'RG': 'E', 'BATCHGRP': 'O_ASIA', 'VWDB': 'EIS', 'SRC_REGION_TABLE': 'ADDR_PRA', 'TGT_REGION_TABLE': 'OutbndShpmtHdr_P', 'DBNAME': 'EIS_STG_T', 'ETLDB': 'EIS_ETL'}
)
