...
def main():
    ...
    for dir in os.walk(workflows_dir).next()[1]:
        ...
        if dir.startswith(workflow_dir_prefix):
            ...
            file_suffix = ""
            for file in os.listdir(os.path.join(workflows_dir, dir)):
                ...
                if re.match("[0-9]+_", file):
					...
					########################################################################
                    if not(file.endswith('.py')):    file_suffix = file.rpartition('.')[2]
					########################################################################
            ...
            for profile, params in task_params_dict.items():
                ...
                if file_suffix == "sql":
					...
					generate_dag(dag_name=dag_name, script_files=script_files,
                                 dag_template="SessionMainDagTemplate.py.jinja", task_params=params,workflow_name=workflow_name)
...
def generate_operator_snippet(script_file, task_params, workflow_name):
	...
	###########################################################################################################################
        ...
        snippet=j2_env.get_template('BashOperator.py.jinja').render(
                task_id=get_task_id(script_file),
                Bash_command=s3_code_directory + '/' + workflow_dir_prefix + workflow_name + '/' + script_file,
				parameters=task_params
		)
	###########################################################################################################################			
    return snippet;
...
def generate_trigoperator_snippet(trig_params,task):
	snippet=j2_env.get_template('DagRunOperator.py.jinja').render(
