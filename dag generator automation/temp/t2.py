...
def main():
	...
	trig_params_dict = {"": None}
		...
		########################################################################
                    if (file.endswith('.slq') or file.endswith('.tpt') or file.endswith('.bteq')):
		        file_suffix = file.rpartition('.')[2]
		########################################################################
		    ...
		    #generate_dag(dag_name=dag_name, script_files=script_files,dag_template="MainDagTptTemplate.py.jinja", task_params=params,workflow_name=workflow_name)
                    #success_audit_sql = get_success_audit_sql(tgt_table_name, actual_table_name, dbname, file_suffix, "None")
                    #failure_audit_sql = get_failure_audit_sql(tgt_table_name, actual_table_name, file_suffix, "None")
                    #generate_dag(dag_name=dag_name, script_files=script_files,
...
def generate_dag(dag_name, script_files, dag_template, task_params, trig_parameter, profile_param, workflow_name):
    print '*******************          generte dag     **************************'
