UPDATE {{SCHEMA_T}}.A_ETL_TABLE_AUDIT A_ETLTblAudit
SET 
...
From    
     ...
     from    {{SCHEMA_STG}}.ECH_ERROR_WARNINGS_REC
     Where    Exception_Severity=1 AND Audit_FK=(Select    ETL_TABLE_AUDIT_PK
     from    {{SCHEMA_T}}.A_ETL_TABLE_AUDIT where table_name='{{PARAMTGTTABLE}}' and TBL_ETL_STATUS='In Progress')) Sev1,
     (Select    Count(*)  NBR_OF_WARNINGS
     from    {{SCHEMA_STG}}.ECH_ERROR_WARNINGS_REC 
     Where    Exception_Severity in(2,3) AND Audit_FK=(Select    ETL_TABLE_AUDIT_PK
     from    {{SCHEMA_T}}.A_ETL_TABLE_AUDIT where table_name='{{PARAMTGTTABLE}}' and TBL_ETL_STATUS='In Progress')) Sev2,
     (Select    Count(*) Nbr_Of_Source_Rows  
     from    {{SCHEMA_STG}}.{{PARAMTGTTABLE}}  WHERE AUDITFK IN (Select  max(ETL_TABLE_AUDIT_PK)  
     from    {{SCHEMA_T}}.A_ETL_TABLE_AUDIT 
     where     Table_Name='{{PARAMTGTTABLE}}' and Tbl_ETL_Status = 'In Progress' )) XI,
     (Select    *  
     from    {{SCHEMA_T}}.A_ETL_TABLE_AUDIT 
     where    ETL_TABLE_AUDIT_PK  In
     (Select    Max(ETL_TABLE_AUDIT_PK) 
     from    {{SCHEMA_T}}.A_ETL_TABLE_AUDIT 
     where    Table_Name='{{PARAMTGTTABLE}}' and Tbl_ETL_Status = 'In Progress' )) A) SrcFields
