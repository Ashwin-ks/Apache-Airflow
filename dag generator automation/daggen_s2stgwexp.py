...
def get_trig_params(file):
	...
	if each_section=="TRIGGERPARAM":
	    ...
	    for (each_key, each_val) in config.items(each_section):
	        print ("each key,each value = " + each_key.upper() , each_val)
		section_params.update({each_key.upper(): each_val})
	        trig_params.update({each_section: section_params})
                trig_params=trig_params['TRIGGERPARAM']
    return trig_params
	...
	trig_params_dict = {"": None}
		...
		########################################################################
                    ...
                    task_params_dict.clear()
              	    trig_params_dict.clear()
                    task_params_dict.update(get_task_params(workflows_dir + '/' + dir + '/' + file))
		    trig_params_dict.update(get_trig_params(workflows_dir + '/' + dir + '/' + file))##
                    print 'printing the dags params', task_params_dict
		    print 'printing the trigger params', trig_params_dict##
            ...
            for profile, params in task_params_dict.items():
                ...
                if params is None or "args" == profile.strip() or profile == "Exp":
                    tgt_table_name = re.split('\.|\$!', workflow_name)[1]
                    actual_table_name = re.split('\.|\$!', workflow_name)[1]
                    dbname = 'EIS_T'
                #else:
                ...
                if file_suffix == "sql":
		     generate_dag(dag_name=dag_name, script_files=script_files,dag_template="SessionMainDagTemplate.py.jinja", task_params=params,trig_parameter=trig_params_dict,workflow_name=workflow_name,profile_param=profile)
                    ...
                    #failure_audit_sql = get_failure_audit_sql(tgt_table_name, actual_table_name, file_suffix, "None")
                    generate_dag(dag_name=dag_name, script_files=script_files,
                                 dag_template="MainDagTptTemplate.py.jinja", task_params=params,workflow_name=workflow_name,profile_param=profile,tgt_table_name=tgt_table_name)
...
def generate_operator_snippet(script_file, task_params, workflow_name):
	...
	###########################################################################################################################			
    elif script_file.endswith(".sh") or script_file.endswith(".ksh"):
        snippet=j2_env.get_template('ExportBashOperator.py.jinja').render(
                task_id=get_task_id(script_file),
		SUBSCRIBERID=task_params['SUBSCRIBERID'],
		DATASETID=task_params['DATASETID']
        )
    return snippet;

def generate_trigoperator_snippet(trig_params,task,profile_param):
    snippet=j2_env.get_template('SessionDagRunOperator.py.jinja').render(
		TRIGGER_DAG_ID=trig_params['TRIGGER_DAG_ID'],
		REGION=profile_param,
...
def generate_dag(dag_name, script_files, dag_template, task_params,workflow_name,trig_parameter={},profile_param=None,tgt_table_name=''):
    print '*******************          generte dag     **************************'
    ...
    for i in range(1, len(task_order_details.keys()) + 1):
        ...
        if i == len(task_order_details.keys()) and "TRIGGER_DAG_ID" in trig_parameter:
            print '??????????????????????????????????????????????'
            dag+=generate_trigoperator_snippet(trig_parameter,all_tasks,profile_param)  
	############################################
    ...
    for i in range(1, len(task_order_details.keys()) + 1):
        ...
        for task in task_list:
            if len(task_list) > 1:
		if i == len(task_order_details.keys()) and profile_param=="Exp":
